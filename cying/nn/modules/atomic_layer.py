...
from .equivariant_spectral_conv import EquivariantSpectralConv3d, EFNO3d
...
class AtomicLayer3d(nn.Module):
    def __init__(self, size, in_channels, out_channels, hidden_width, spe_opt_size, spa_opt_size):
        ...
        self.norm_lin = nn.RMSNorm(size)

    def forward(self, input):
        lin_opts = self.spectral_opts(input)
        return self.activate(self.norm_lin(lin_opts)) + self.linear_skip(input)